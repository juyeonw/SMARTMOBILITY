...
if(add_selectbox == "체질량 계산기"):
    ...
    age = st.slider('나이를 선택하세요.', 0, 130, 22)
    st.write("저의 나이는", age, '입니다.')

    st.write('# 체질량 계산기')
    st.success('체질량 지수: 몸무게/((키/100)^2)')
    height = st.number_input('키를 입력하세요(cm)',100,230,167,5)
    st.write('당신의 키는', height, 'cm 입니다.')

    weight = st.number_input('몸무게를 입력하세요(kg)',40,150,50,5)
